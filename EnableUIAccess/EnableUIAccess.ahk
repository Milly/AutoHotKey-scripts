/*
    EnableUIAccess.ahk v1.00 by Lexikos
...
EXE_uiAccess_set(file, value)
{
    ...
    node := xml.selectSingleNode("/assembly/trustInfo/security"
    ...
    if !node
    {
        ...
        if !last := xml.selectSingleNode("/assembly")
        {
        ...
        {
            if !(node := last.selectSingleNode(name))
            {
